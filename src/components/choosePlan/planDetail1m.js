            ...
            <Form onSubmit={handleSubmitGold}>
              <div className='row m-2'>
                ...
                <div className='col-md-4 col-xs-12 plan-info'>
                  ...
                  <p className="text-dark">Rendimeto Liveb 1.5% A.m: <strong>R${investimento == null ? 0 : (((valor * (quantidadeCotasComparador)) * 1.5) / 100).toFixed(2)
                    .replace('.', ',')
                    .replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1.')}</strong></p>
                  <p className="text-dark">Rendimeto CDI 0.22% A.m: <strong>R${investimento == null ? 0 : (((valor * quantidadeCotasComparador) * 0.22) / 100).toFixed(2)
                    .replace('.', ',')
                    .replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1.')}</strong></p>

                  <p className="text-dark">Rendimeto Poupanca 0.16% A.m: <strong>R${investimento == null ? 0 : (((valor * quantidadeCotasComparador) * 0.16) / 100).toFixed(2)
                    .replace('.', ',')
                    .replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1.')}</strong></p>
                  <br />
                  <Form.Group>
                    <Form.Text className="text-muted pb-2">Em qual opção gostaria de investir?</Form.Text>
